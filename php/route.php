...
1) bad
$logMessage = 'A '.$user->type.' with e-mail address '.$user->email.
              ' has performed '.$action.' on '.$subject.'.';

2) good
$logMessage = sprintf('A %s with email %s has performed %s on %s.', 
                $user->type, $user->email, $action, $subject);

3) better
$logMessage = "A {$user->type} with e-mail address {$user->email} has performed {$action} 
               on {$subject}.";
*/
