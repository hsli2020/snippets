...
// Command line flags.
var requestUrl = flag.String("request_url", "",	"Host and port to send the request to")
var requestMethod = flag.String("request_method", "GET", "HTTP request method")
...
func doRequest(requestMethod, requestUrl, requestData string) (*http.Response, error) {
	...
	case "DELETE", "PATCH", "PUT":
		...
		if len(requestData) == 0 && upperRequestMethod != "DELETE" {
			...
			return nil, fmt.Errorf("--request_data must be provided")
		}
		...
		req, err := http.NewRequest(upperRequestMethod, requestUrl,	contentBuffer)
		if err != nil {
			// Failed creating HTTP request.
			return nil, fmt.Errorf("error creating new HTTP request")
		}
	...
	case "POST":
		...
		res, err = http.Post(requestUrl, "application/json", contentBuffer)

	default:
		// We doń't know how to handle this request.
		return nil, fmt.Errorf("invalid --request_method provided : %s", requestMethod)
	}
