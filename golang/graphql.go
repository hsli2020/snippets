...
func (g *GraphQL) Execute(ctx context.Context, graphql string, response interface{}, 
		variables ...func(m map[string]interface{})) error {
...
func (g *GraphQL) ExecuteOnEndpoint(ctx context.Context, endpoint string, graphql string, 
		response interface{}, variables ...func(m map[string]interface{})) error {
	var queryVars map[string]interface{}
	if len(variables) > 0 {
		queryVars = make(map[string]interface{})
		for _, variable := range variables {
...
// around the query and variables. Then executes the request against the configured url/endpoint.
func (g *GraphQL) query(ctx context.Context, endpoint string, graphql string, 
		queryVars map[string]interface{}, response interface{}) error {
	request := struct {
...
// RawRequest performs the actual execution of a request against the specified url/endpoint. 
// Use this function only when the request doesn't require a graphql document wrapper.
func (g *GraphQL) RawRequest(ctx context.Context, endpoint string, r io.Reader, response interface{}) error {
	...
	if len(result.Errors) > 0 {
		return fmt.Errorf("graphql op error: request:[%s] error:[%s]",
			request.String(), result.Errors[0].Message)
	}
