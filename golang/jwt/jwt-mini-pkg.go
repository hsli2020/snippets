package jwt // https://github.com/nilslice/jwt
...
// New returns a token (string) and error. The token is a fully qualified JWT to be sent to
// a client via HTTP Header or other method. Error returned will be from the newEncoded
// unexported function.
...
// Passes returns a bool indicating whether a token (string) provided has been signed by
// our server. If true, the client is authenticated and may proceed.
...
// Secret is a helper function to set the unexported privateKey variable used when signing
// and verifying tokens.
// Its argument is type []byte since we expect users to read this value from a file which
// can be excluded from source code.
