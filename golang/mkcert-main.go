...
func (m *mkcert) Run(args []string) {
	...
	if m.installMode {
		...
		if !m.checkPlatform() {
			warning = true
			log.Println("Warning: the local CA is not installed in the system trust store! ??")
		}
		if hasNSS && !m.checkNSS() {
			warning = true
			log.Printf("Warning: the local CA is not installed in the %s trust store! ??", NSSBrowsers)
		}
		if warning {
