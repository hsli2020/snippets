...
// postcardHandler http handler parse user input and generate PNG postcard in response
// by calling `generatePostcard`
...
// generatePostcard creates image from postcard background, draw stamp and `lines`
// strings using `clarendonBT` font
func generatePostcard(lines []string) image.Image {
	...
	draw.Draw(postcard, stampImg.Bounds().Add(image.Point{835, 45}), stampImg,
