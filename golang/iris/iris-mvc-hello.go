...
// this example does not make use of iris mvc features like
// the Model, Persistence or the View engine neither the Session,
// it's very simple for learning purposes, probably you'll never use such
// as simple controller anywhere in your app.
//
// The cost we have on this example for using MVC on the "/hello" path which serves JSON
// is ~2MB per 20MB throughput on my personal laptop, it's tolerated for the majority of
// the applications but you can choose what suits you best with Iris, low-level handlers:
// performance or high-level controllers: easier to maintain and smaller codebase on large
// applications.
...
func newApp() *iris.Application {
	app := iris.New()
	// Optionally, add two built'n handlers that can recover from any http-relative panics
...
func (c *ExampleController) All() {}    // OR
