// Package handler contains an HTTP Cloud Function to handle update from
// Telegram whenever a users interacts with the bot.
package handler // https://github.com/fpaupier/telegrap
...
var rapLyricsApi string = os.Getenv("RAPLYRICS_API")
...
// HandleTelegramWebHook sends a message back to the chat with a punchline starting by
...
func HandleTelegramWebHook(w http.ResponseWriter, r *http.Request) {
	...
	if errTelegram != nil {
		...
		log.Printf("punchline %s successfully distributed to chat id %d",
			lyric, update.Message.Chat.Id)
	}
}
...
func parseTelegramRequest(r *http.Request) (*Update, error) {
	...
	if update.UpdateId == 0 {
		...
		// 0 indicates failure to parse incoming update")
	}
	return &update, nil
}
...
func getPunchline(seed string) (string, error) {
	rapLyricsResp, err := http.PostForm(rapLyricsApi,
...
func sendTextToTelegramChat(chatId int, text string) (string, error) {
	...
	response, err := http.PostForm(telegramApi,
		url.Values{
