// Package imapsync provides a single function to download an IMAP folder to a local directory,
// with each email in a plain text file. Emails are downloaded only once, even if the function
// is run repeatedly.
//
...
// Result contains slices of (relative) paths to the newly (NewEmails) and previously downloaded emails
// (ExistingEmails). Only emails still present on the server will be returned.
type Result struct {
