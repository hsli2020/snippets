...
contract XENFT is IXENTorrent, IXENProxying, ERC721("XENFT", "XENFT") {
    ...
    /**
     * @dev function callable only in proxy contracts from the original 
     * one => XENCrypto.claimMintRewardAndShare()
     */
    function callClaimMintReward(address to) external {
        require(msg.sender == _original, "unauthorized");
    ...
    /**
     * @dev function callable only in proxy contracts from the original one => destroys the
     * proxy contract
     */
    function powerDown() external {
        require(msg.sender == _original, "unauthorized");
    ...
    /**
     * @dev main torrent interface. initiates Mint Reward claim and collection and terminates
     * Torrent Operation
     */
    function bulkClaimMintReward(uint256 tokenId, address to) external {
