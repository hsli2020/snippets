...
class Modal extends React.Component {
	...
	componentDidUpdate() { }
  	...
  	_mouseMove(event) {
  		...
  		if (modal.offsetLeft + modal.offsetWidth + diffX >= document.documentElement.clientWidth ||
            dropLeft <= 0) {
  			return;
  		}
  		if (modal.offsetTop + modal.offsetHeight + diffY >= document.documentElement.clientHeight ||
            dropTop <= 0) {
  			return;
  		}
	...
	render() {
		...
		return (
			this.props.isOpen ?
				<div>
					<div className='modal' ref='modal' style={styles.modal}
