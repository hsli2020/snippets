...
const obfuscateOpenAIResponse = (text) => 
    text.replace(/\borg-[a-zA-Z0-9]{24}\b/g, 'org-************************')
        .replace(' Please add a payment method to your account to increase your rate limit. 
...
app.post('/v1/', async (req, res) => {
  ...
  if (!contentType || contentType !== 'application/json') {
    return res.status(415)
          .set(corsHeaders)
          .type('text/plain')
          .send("Unsupported media type. Use 'application/json' content type");
  }
  ...
  if (stream != null && stream !== true && stream !== false) {
    return res
          ...
          .set(corsHeaders)
          .type('text/plain')
          .send('The `stream` parameter must be a boolean value');
  }
